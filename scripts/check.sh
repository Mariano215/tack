...
out="$(bash scripts/test-marketplace-enabled.sh 2>&1)" || { echo "$out"; err "scripts/test-marketplace-enabled.sh failed." "The enabled-marketplace guard in hooks/plugin-auto-update.sh changed. Without it, a disabled plugin still pays a cold install on every session start, and on Windows that install fails with ENOTEMPTY invisibly."; }
